...
module.exports = {
    ...
    addNewEmptyUserChats:  (req, res) => {
        ...
        chatRef.get()
            ...
            .catch((err) => {
                return res.status(404).send("Errore di comunicazione con il server")
            });
    },
                ...
                firestone.collection('usersChats').doc(currentUserId)
                    ...
                    .then((r) => {
                        ...
                        firestone.collection('usersChats').doc(userToChatId)
                            ...
                            .catch((err) => {
                                res.status().send("(sender) Errore nella comunicazione col server",err);
                            })
                    })
                    .catch((err) => {
                        res.status(500).send("(receiver) Errore nella comunicazione col server",err);
                    })
            })
            .catch((err) => {
                res.status(500).send("(addMessage) Errore nella comunicazione col server" + err);
            })
    }
}