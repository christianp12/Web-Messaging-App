...
module.exports = {
    ...
    //Restituisce le richieste ricevute in attesa (in un array)
    getPendingReceivedFriendRequests: async (req,res) => {
    ...
    //Restituisce le richieste ricevute ed accettate (in un array)
    getAcceptedReceivedFriendRequests: async (req,res) => {
        const snapshot = await firestone.collection('friendRequests')
            .where('toUserId', '==', req.params.uid)
    ...
    getPendingSentFriendRequests: async (req,res) => {
        const snapshot = await firestone.collection('friendRequests')
    ...
    getAcceptedSentFriendRequests: async (req,res) => {
        const snapshot = await firestone.collection('friendRequests')
            .where('fromUserId', '==', req.params.uid)
            .where('status', '==',  'accepted')
            .get();
    ...
    deleteFriendRequest: async (req,res) => {
        ...
        try{
            await firestone.collection('friendRequests').doc(requestId).delete();
            res.send(`Friends request has been deleted successfully.`)
        } catch (error) {
